 ...
 * Sets the macroscopic variables (density `rho`, velocity `v`) and the
 * mesoscopic distribution functions (`f`) to their initial states based on a
 * chosen pattern.
 */
void initialize_lbm(int global_Nx, int global_Ny, int local_Nx, int local_Ny,
                    int offset_x, int offset_y, ScalarField2D &rho,
                    VectorField2D &v, DistFuncD2Q9 &f, DistFuncD2Q9 &f_next,
    ...
    // The index 0 and local_N+1 are reserved for halo/ghost cells for MPI
    // communication.

    if (pattern == InitialisationPattern::Empty) {
        // Kokkos::parallel_for with MDRangePolicy launches a 2D parallel loop
        // on the GPU/CPU.
        Kokkos::parallel_for(
            "InitFields",
            Kokkos::MDRangePolicy<Kokkos::Rank<2>>(
                {1, 1}, {local_Nx + 1, local_Ny + 1}),
            KOKKOS_LAMBDA(const int x, const int y) {
                ...
                // Initialize distribution function f_i to equilibrium at rest
                // (which is just w_i * rho)
                for (int i = 0; i < 9; ++i)
                    f(x, y, i) = w[i];
            });
        ...
        Kokkos::parallel_for(
            ...
            KOKKOS_LAMBDA(const int x, const int y) {
                ...
                // Calculate global coordinates to ensure smooth wave across MPI
                // boundaries
                int gx = offset_x + (x - 1);
                int gy = offset_y + (y - 1);
                ...
                // Initialize f to the local equilibrium distribution f^eq(rho,
                // u)
                for (int i = 0; i < 9; ++i) {
                    double cu = cx[i] * vx + cy[i] * vy;
                    // Standard LBM equilibrium formula: w_i * rho * (1 + 3(c.u)
                    // + 4.5(c.u)^2 - 1.5(u)^2)
...
/**
 * Streaming
 * Moves particle distributions to neighboring cells based on their discrete
 * velocities pull scheme (f_out reads from f_in's neighbors) avoids data races
 * in parallel execution
 *
 * f_in/f_out: Particle Distribution Function (mesoscopic). array of 9
 * numbers per grid cell. It represents the probability or density of fluid
 * particles moving in direction i at that specific cell.
 *
 * bounce: boolean indicating if the fluid should bounce from the walls
 *
 * u_lid: if bounce from top wall, injext momentum: +6.0*w_i*cx_i*u_lid
 *
 * offset_x/_y: where start globaly
 *
 * cell_type: array indicating if a cell is sink, source or wall
 */
void streaming(DistFuncD2Q9 f_in, DistFuncD2Q9 f_out, bool bounce, double u_lid,
    ...
    const double w[9] = {4. / 9.,  1. / 9.,  1. / 9.,  1. / 9., 1. / 9.,
                         1. / 36., 1. / 36., 1. / 36., 1. / 36.};
    // Map to the opposite direction (e.g., East (1) opposes West (3)) for
    ...
    Kokkos::parallel_for(
        ...
        KOKKOS_LAMBDA(const int x, const int y) {
            ...
            for (int i = 0; i < 9; ++i) {
                ...
                // Source cell in the global domain (used for global bounds
                // checks)
                ...
                // Check global domain boundaries if global bounce-back is
                // enabled
                if (bounce && (global_src_x < 0 || global_src_x >= global_Nx ||
                    ...
                    if (src_x >= 1 && src_x <= local_Nx && src_y >= 1 &&
                        src_y <= local_Ny && cell_type(src_x, src_y) == 1) {
                        is_wall = true;
                    }
                }

                if (is_wall) {
                    // Half-way bounce-back: Particle reflects back where it
                    // came from (opp[i])
                    ...
                    // Normal streaming: Read from local array (which includes
                    // halo/ghost cells if src is on boundary)
    ...
    Kokkos::parallel_for(
        ...
        KOKKOS_LAMBDA(const int x, const int y) {
            ...
            for (int i = 0; i < 9; ++i) {
                ...
                double feq = w[i] * rho_val *
                             (1.0 + 3.0 * cu + 4.5 * cu * cu - 1.5 * usq);

                // BGK Collision operator: f_new = f_old - omega * (f_old - feq)
                // Omega is the relaxation parameter inversely proportional to
                // kinematic viscosity.
...
/**
 * Calculates Macroscopic Properties.
...
void compute_velocity(ScalarField2D rho, VectorField2D v, DistFuncD2Q9 f,
    ...
    Kokkos::parallel_for(
        ...
        KOKKOS_LAMBDA(const int x, const int y) {
            ...
            for (int i = 0; i < 9; ++i) {
                double f_i = f(x, y, i);
                local_rho += f_i; // sum up distribution function values

                // we summ up:
                // positive if moving right/up
                // negetive if moving left/down
                // 0 if no change
                // this gets us x- and y- momentum
                local_vx += f_i * cx[i];
                local_vy += f_i * cy[i];
            }

            rho(x, y) = local_rho;
            // divide the momentum by the density to get the velosity
...
/**
 * Forces boundary constraints inside the domain (e.g., fluid
 * injection/extraction).
 *
 * Overwrites the distribution functions at source/sink locations.
 */
void apply_sources(DistFuncD2Q9 f, CellTypeField cell_type, int local_Nx,
                   int local_Ny) {
    Kokkos::parallel_for(
        "ApplySources",
        Kokkos::MDRangePolicy<Kokkos::Rank<2>>({1, 1},
        ...
        KOKKOS_LAMBDA(const int x, const int y) {
            ...
            if (type == 2) {
                // SOURCE: Force a constant high density (rho = 1.1), velocity =
                // 0 This will push fluid outward into the domain.
                for (int i = 0; i < 9; ++i)
                    f(x, y, i) = w[i] * 1.1;
            } else if (type == 3) {
...
double get_max_speed(int local_Nx, int local_Ny, ScalarField2D speed) {
    ...
    Kokkos::parallel_reduce(
        ...
        Kokkos::Max<double>(
            max_val)); // Kokkos reducer ensures thread-safe max computation
    return max_val;
}
...
void speed_to_rgb(int local_Nx, int local_Ny, ScalarField2D speed,
                  PixelField rgb, double max_speed) {
    Kokkos::parallel_for(
        "SpeedToRGB",
        Kokkos::MDRangePolicy<Kokkos::Rank<2>>({1, 1},
                                               {local_Nx + 1, local_Ny + 1}),
        KOKKOS_LAMBDA(const int x, const int y) {
            ...
            if (s > 1.0) {
                // Overexposed (can happen between frame max_speed updates) ->
                // White, to remove flickering i implemented a 30 frame moving
                // average, thus the pixels can be overexposed
                rgb((y - 1) * local_Nx + (x - 1)) =
 ...
 */
void velocity_dir_to_rgb(int local_Nx, int local_Ny, VectorField2D v,
                         PixelField rgb, double max_speed) {
    Kokkos::parallel_for(
        "VelocityDirToRGB",
        Kokkos::MDRangePolicy<Kokkos::Rank<2>>({1, 1},
        ...
        KOKKOS_LAMBDA(const int x, const int y) {
            ...
            if (mag > 1e-6 && max_speed > 1e-6) {
                ...
                if (v_val > 1.0) {
                    ...
                    // Map the 2D direction angle (-PI to PI) to Hue (0.0
                    // to 1.0) The a circular hue with west red, north green,
                    // east green/blue (turquoise), south blue/pink
                    double theta = Kokkos::atan2(vy, vx);
                    double hue = (theta + 3.14159265358979323846) /
                    ...
                    // Standard HSV to RGB conversion logic (Saturation is fixed
                    // to 1.0)
    ...
    Kokkos::parallel_for(
        ...
        KOKKOS_LAMBDA(const int x, const int y) {
            ...
            if (intensity > 1.0) {
                ...
                rgb_density((y - 1) * local_Nx + (x - 1)) =
                    (255 << 24) | (255 << 16) | (255 << 8) | 255;
            } else {
                if (intensity < 0.0)
                    intensity = 0.0;

                // Blue (low density) to Red (high density) colormap
                // interpolation
