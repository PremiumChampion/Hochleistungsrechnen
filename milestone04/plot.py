...
if os.path.exists("velocity_profile.csv"):
    ...
    ax2.set_title("Analytical Error")
    ...
    print(f"\n---> Global Mean Absolute Error: {np.mean(all_mae):.4e} (lu/ts)\n")
...
if os.path.exists("viscosity.csv"):
    ...
    # Create 1x2 subplots instead of a twin y-axis
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(13, 6))
    fig.suptitle("Measured vs Analytical Kinematic Viscosity", fontsize=14, fontweight='bold')
    
    # ---------------------------------------------------------
    # Left Subplot: Viscosities
    # ---------------------------------------------------------
    ax1.plot(df_visc["omega"], df_visc["measured_nu"], 'o-', 
             color="tab:blue", label="Measured Numerical Viscosity")
    ax1.plot(df_visc["omega"], df_visc["analytical_nu"], 's--', 
    ...
    ax1.set_title("Viscosity Comparison")
    ax1.grid(True, linestyle="--", alpha=0.6)
    ax1.legend(loc="upper right")
    
    # ---------------------------------------------------------
    # Right Subplot: Absolute Difference (Error)
    # ---------------------------------------------------------
    delta_nu = (df_visc["measured_nu"] - df_visc["analytical_nu"]).abs()
    
    ax2.plot(df_visc["omega"], delta_nu, 'd:', 
             color="tab:red", label=r"Absolute Difference $|\Delta \nu|$")
    
    ax2.set_xlabel(r"Relaxation Parameter ($\omega$) (dimensionless)")
    ax2.set_ylabel(r"Absolute Difference $|\Delta \nu|$ (lu$^2$/ts)", color="tab:red")
    ax2.tick_params(axis='y', labelcolor="tab:red")
    ax2.set_title("Viscosity Error")
    ax2.grid(True, linestyle="--", alpha=0.6)
    ...
    # Adjust layout to fit titles, subplots, and the glossary box
    plt.subplots_adjust(top=0.88, bottom=0.22, wspace=0.25)
