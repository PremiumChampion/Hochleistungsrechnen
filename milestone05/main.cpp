...
int main(int argc, char *argv[]) {
...

    {
        ...
        if (rank == 0) {
            std::cout
                << "Starting lid-driven cavity simulation (Re ~ 435)...\n";
        }
        ...
        if (rank == 0)
            std::cout << "Converged to steady-state after " << step
                      << " steps.\n";
        ...
        if (rank == 0) {
            ...
            std::cout << "Centerline velocity profile saved to "
                         "centerline_profile.csv\n";
        }
    }
    Kokkos::finalize();
    MPI_Finalize();
